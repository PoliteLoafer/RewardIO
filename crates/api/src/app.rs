...
mod tests {
    ...
    #[async_trait]
    impl rewardio_core::MessageService for TestMessageService {
        async fn get_hello_message(
            &self,
        ) -> Result<rewardio_core::Message, rewardio_core::RepositoryError> {
            Ok(rewardio_core::Message {
    ...
    async fn test_app_new_and_router() {
        ...
        let state = AppState {
            message_service,
            auth_service,
            session_manager: Arc::new(crate::auth::session::SessionManager::new(
                &config.auth_secret,
